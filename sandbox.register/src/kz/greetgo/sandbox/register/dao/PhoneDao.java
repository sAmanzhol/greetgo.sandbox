...
public interface PhoneDao {
    ...
    @Select("insert into client_phone(id,client,number,type,actual) VALUES (DEFAULT,#{phone.client},#{phone.number},#{phone.type}::phone_type,true) " +
            "RETURNING id")
    ...
    @Select("select * from client_phone where client = #{client_id} and actual = true")
    ...
    @Select("update client_phone set client = #{phone.client},number = #{phone.number},type = #{phone.type}::phone_type WHERE id = #{phone.id} and actual = true RETURNING *")
